...
def filter_anomalies(signal_segments, n_mad=3):
    rms_values = np.array([np.sqrt(np.mean(segment ** 2)) for segment in signal_segments])
    median_rms = np.median(rms_values)
    mad = np.median(np.abs(rms_values - median_rms))
    threshold_low = median_rms - n_mad * mad
    threshold_high = median_rms + n_mad * mad
...
for material, df in dataframes.items():
    ...
    # 通过 ISTFT 得到降噪后的时域信号
    _, denoised_signal = istft(Zxx_denoised, fs=fs, nperseg=4096)
    ...
    plt.legend()
    plt.savefig(
        f"C:/Users/c1257/Desktop/data_diagram/processed_data_comparison/BWiener/{material}_time_comparison_wiener.png")
    plt.show()
    ...
    plt.ylim(0, 20000)
    plt.savefig(
        f"C:/Users/c1257/Desktop/data_diagram/processed_data_comparison/BWiener/{material}_original_stft.png")
    plt.show()
    ...
    plt.ylim(0, 20000)
    plt.savefig(
        f"C:/Users/c1257/Desktop/data_diagram/processed_data_comparison/BWiener/{material}_denoised_stft_wiener.png")
    plt.show()
    ...
    df_psd = pd.DataFrame({'frequency': frequencies_denoised, 'psd': psd_denoised})
    df_psd.to_csv(f"C:/Users/c1257/Desktop/processed_data/psd_Band_Wiener_{material}.csv", index=False)
    ...
    plt.legend()
    plt.savefig(
        f"C:/Users/c1257/Desktop/data_diagram/processed_data_comparison/BWiener/{material}_psd_comparison_wiener.png")
    plt.show()
    ...
    df_denoised.to_csv(f"C:/Users/c1257/Desktop/processed_data/processed_data_BWiener/denoised_{material}.csv",
                       index=False)
...
df_features.to_csv("C:/Users/c1257/Desktop/processed_data/processed_data_BWiener/feature_data.csv", index=False)

# Convert statistics summary to DataFrame and visualize
stats_df = pd.DataFrame(stats_summary).T
print(stats_df)
stats_df.to_csv("C:/Users/c1257/Desktop/processed_data/processed_data_BWiener/signal_statistics.csv")

# Plot statistical summary
stats_df.plot(kind='bar', figsize=(12, 6), title="Statistical Features of Denoised Signals")
plt.ylabel("Value")
plt.savefig(
    "C:/Users/c1257/Desktop/data_diagram/processed_data_comparison/BWiener/statistical_features_wiener.png")
plt.show()
